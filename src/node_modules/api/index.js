...
export const fetchRecipes = async (allowedIngredient, excludedIngredient, cuisine, course, maxTotalTime) => {  
  ...
  const {body} = await request.get(
    `http://api.yummly.com/v1/api/recipes?_app_id=${appId}&_app_key=${appKey}${allowedIngredientList}${excludedIngredientList}${cuisineList}${courseList}${maxTotalTimeList}&requirePictures=true`
  )

  if (body.matches.length !== 0)
    inredientsToMore = `${allowedIngredientList}${excludedIngredientList}${cuisineList}${courseList}${maxTotalTimeList}`
