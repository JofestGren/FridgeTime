...
class Search extends Component {
  ...
  handleSearchClick (event) {
    ...
    const checkInputCorrect = (value) => {
      const reg = /^[a-zA-Z\s,]+$/ ///[a-zA-Z\s,]{1,}/
      const tempArr = value.split(',')
      return tempArr.some(element => {
        const isCheckTrue = reg.test(element)
        console.log(isCheckTrue)
        return isCheckTrue === false
      })
      // let flag = false
      // for (let j = 0; j < tempArr.length; j++) {
      //   const arr = [...tempArr[j]]
      //   for (let i = 0; i < arr.length; i++) {
      //     const isCheckTrue = reg.test(arr[i])
      //     if (!isCheckTrue) {
      //       flag = true
      //       break
      //     }
    ...
    const checkInput = (value, nameInput) => {
      if (value !== '') {
        const isSymbol = checkInputCorrect(value)
    ...
    const checkInputNumberCorrect = (value) => {
      const reg = /^\d+$/ // /[0-9\S]/
    ...
    const checkInputNumber = (value, nameInput) => {
      if (value !== '') {
        const isSymbol = checkInputNumberCorrect(value)
        console.log('isSymbolNumber ', isSymbol)
        if (!isSymbol) {
          this.setState({
            [`${nameInput}Error`]: true,
            [`${nameInput}`]: ''
          })
          console.log('fun', this.state.maxTotalTimeError)
        } else {
          this.setState({
            [`${nameInput}Error`]: false,
            [`${nameInput}`]: value
